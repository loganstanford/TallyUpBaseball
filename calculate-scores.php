...
<head>
    ...
    function getTeam1(teamId) {
        ...
        xmlhttp.open("GET", "getTeam.php?id=" + teamId + "&date=" + date + "&opp=1", true);
        xmlhttp.send();
    }
    ...
    function getTeam2(teamId) {
        ...
        xmlhttp.open("GET", "getTeam.php?id=" + teamId + "&date=" + date + "&opp=2", true);
        xmlhttp.send();
    }
    ...
    function calculateColumn(index, id) {
        ...
        $(`#opp-${id} > tfoot #total`)[0].value = total;
    }

    function updatePoints(points, position, id) {
        $(`#opp-${id} > tbody tr > #${position}-points`)[0].innerHTML = points.substring(points.indexOf('&') + 1);
        ...
        <div class="content-wrapper">
            ...
            <section class="content">
                <div class="container-fluid">
                    <form action="includes/matchup.inc.php" method="post">
                        <div class="input-group date" id="matchup-date" style="cursor: pointer"
                            data-target-input="nearest">
                            <label class="mx-2">Select date:</label>
                            <input type="text" name="date" class="form-control datetimepicker-input"
                                id="matchup-date-input" style="cursor: pointer" data-target="#matchup-date"
                                data-toggle="datetimepicker" oninput="enableTeams()" />
                            <div class="input-group-append" data-target="#matchup-date" data-toggle="datetimepicker">
                                <div class="input-group-text"><i class="fa fa-calendar"></i><i
                                        class="fas fa-caret-down px-2"></i></div>
                            </div>
                        </div>
                        <div class="row">
                            <!-- Opponent 1 Table -->
                            <div class="card col-xl-5 col-md-12 m-2">
                                <div class="card-header">
                                    <select disabled class="form-control" name="team1" id="team1"
                                        onchange="getTeam1(this.value)">
                                    ...
                                    while($t=$q->fetch()): ?>
                                        <option value="<?php echo $t['team_id'];?>">
                                            <?php echo $t['division'] . ' - ' . $t['team_name'];?></option>
                                        <?php endwhile; ?>
                                    </select>
                                </div>
                            ...
                            <div class="card col-xl-5 col-md-12 m-2">
                                <div class="card-header">
                                    <select disabled class="form-control" name="team2" id="team2"
                                        onchange="getTeam2(this.value)">
                        ...
                        if (isset($_SESSION['admin'])) {
                            if ($_SESSION['admin'] = 1) {
                                echo '<button type="submit" class="btn btn-primary" name="submit">Save score</button>';
                            }
                        }
                        ?>
                    </form>
